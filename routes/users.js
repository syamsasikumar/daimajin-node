      ...
      if(collection){
        collection.findOne({name:name, pass:pass}, {_id:1, name:1}, function(err, item){
          ...
          if(item){
            ...
            res.send({code:0, status:'User logged in', _id:item._id, name:item.name, lists: {}, ratings:{}});
          }else{
        ...
        collection.findOne({name:name}, function(err, item){
          ...
          if(item){
            ...
            collection.insert([{name:name, pass:pass}], function(err, rec){
              ...
              res.send({code:0, status:'User registered successfully', _id:rec[0]._id, name:name, lists: {}, ratings:{} });
            })
          }
